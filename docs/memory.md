...
42. Superseded by rule 65: React/FastAPI became the only supported active runtime while the final Streamlit implementation was temporarily retained as rollback evidence.
...
60. Superseded in navigation by rule 61; retain its transient-search boundary, explicit persistence actions, one deduplicated Drive queue, unmerged source records, and unchanged Gmail history.
61. Use **Results** only for the current or explicitly loaded search records and a separate **Applications** tab only for Drive-persisted jobs. Applications must support saved-for-later, preparing, applied/later, and closed views plus editable application status and notes without creating another storage file or duplicating records.
62. Superseded for the applied-job file set by rule 64; retain the explicit manual-confirmation gate, package-before-status ordering, idempotency, and no-submission boundary.
63. Revalidate every company-registry job URL before publishing a registry revision. Color the full row red only for a confirmed inaccessible source, color the full row blue when browser review or a company-specific/manual adapter is required, and never classify an automated access block alone as a dead employer page. Prefer India-relevant official portals and keep product, startup, and mid-sized coverage incremental and mutually exclusive.
64. A job may enter `applied`, `interviewing`, or `offer` through the supported UI only after generated documents exist and the user explicitly confirms manual application. First save a readable `Job_Description.docx`, clean `Job_Description.md`, and machine-readable `Application_Details.json` in the exact generated-resume Drive folder, labeling the JD evidence as full, partial, or summary-only. Never present a summary as a complete JD; then update the canonical queue with `applied_at` and package links. This action never submits an application and must remain idempotent.
65. React/FastAPI is the only application runtime shipped by the repository. The retired Streamlit rollback implementation may be removed as unused code; preserve its numbered historical discussions instead of keeping an executable legacy application.
66. Use `docs/setup/GCP_CLOUD_RUN_DEPLOYMENT.md` as the deployment source of truth. The personal direction is a private single-user Cloud Run service in Mumbai behind IAP, with static secrets in Secret Manager, mutable runtime state on a private persistent mount, Drive remaining the user-artifact source of truth, and one maximum instance while file-backed state remains. Do not execute deployment until the runbook's code gate is complete and the user approves it; do not add scheduling or multi-user scope implicitly, and never commit or request secret values.
